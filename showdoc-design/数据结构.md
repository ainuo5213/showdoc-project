...
| Cellphone  | false    | nvarchar(20)  | false    | 0         | 用户手机号        |
...
创建：用户创建团队时，用户确认邀请函加入团队时创建，一个人仅归属于一个团队。
...
### 团队邀请函 TeamInvitations

创建：用户邀请其他人员加入团队时创建，团队归属人仅能邀请未加入团队的用户。一个用户仅存在一条待加入的邀请函。
...
| 字段名           | 是否主键 | 字段类型 | 是否为空 | 默认值    | 字段备注                               |
| ---------------- | -------- | -------- | -------- | --------- | -------------------------------------- |
| TeamInvitationID | true     | int      | false    | x         | 主键ID。默认自增                       |
| UserID           | false    | int      | false    | ''        | 被邀请者ID                             |
| InvitByID        | false    | Int      | false    | x         | 邀请者ID                               |
| Status           | false    | int      | false    | 0         | 状态。-1：已拒绝；0：待加入；1：已加入 |
| TeamID           | false    | int      | false    | x         | 邀请加入的团队ID                       |
...
| 字段名      | 是否主键 | 字段类型         | 是否为空 | 默认值    | 字段备注                        |
| ----------- | -------- | ---------------- | -------- | --------- | ------------------------------- |
| DocumenrtID | true     | int              | false    | x         | 主键ID。默认自增                |
...
| CreatorID   | false    | uniqueidentifier | false    | x         | 文档创建者ID                    |
| UpdatorID   | false    | uniqueidentifier | false    | x         | 文档最后一次更新者ID            |
...
| 字段名       | 是否主键 | 字段类型      | 是否为空 | 默认值    | 字段备注         |
| ------------ | -------- | ------------- | -------- | --------- | ---------------- |
| VerifyCodeID | true     | int           | false    | x         | 主键ID。默认自增 |
